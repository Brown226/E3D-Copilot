...
const QuickWinCard: React.FC<QuickWinCardProps> = ({ task, onExecute }) => {
	return (
		<div
			className="flex items-center mb-2 py-0 px-5 space-x-3 rounded-full cursor-pointer group transition-colors duration-150 ease-in-out bg-muted/40 border border-border hover:bg-list-hover"
			onClick={() => onExecute()}>
			<div className="shrink-0 flex items-center justify-center w-6 h-6 text-icon-foreground">
				{renderIcon(task.icon)}
			</div>

			<div className="grow min-w-0">
				<h3 className="text-sm font-medium truncate text-foreground leading-tight mb-0 mt-0 pt-3">
					{task.title}
				</h3>
